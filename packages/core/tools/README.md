...
- `ctx.tools.outputProjection(exec)` returns execution-local structured projection metadata when the resolved output declaration used a built-in renderer. Today `{ kind: 'json', schema }` identifies a result rendered by `jsonRenderer`; custom render functions return `undefined`. Policies use this to specialize presentation without parsing rendered text or guessing from content.
...
Use `jsonRenderer({ space: 0 | 2 })` when JSON text is the intended model-facing representation. It is explicit rather than inferred: an arbitrary custom renderer may also emit JSON-looking text, while a declared JSON renderer preserves the relationship between the validated canonical value and `output.schema`. The spill policy uses that relationship to save an oversized result as valid JSON and describe its root without reparsing the rendered string.
